...
{
	int n;
	cout << "Enter new size that is larger of double of previous size : ";
	cin >> n;
	while (n <= (s * 2))
	{
		cout << "Please! Enter new size that is larger of double of previous size : ";
		cin >> n;
	}
	int* temp = new int[n];
	for (int i = 0;i < n;i++)
	...
	p = temp;
	s = n;
//why de-allocation and relocation?  dynamic memory created on heap and when we copy our array in temp then our previous array is wasted and 
//used extra memory in heap therefore first dellocate and then assign the address of new array 
//what if parameters are by value?   If we receive parameters by then we can't change orginal value of parameters only changes will be done in
//copies therefore for changes and modify data permanantly we pass values by reference and alias of these parameters will be created don't use 
//extra memory
}
...
{//this function will shrink array and stored in temp pointer and after deallocate
//pointer that we received will store temp in pointer and after reallocate return whole pointer that 
//points new dynamic array 
	int x;
	cout << "\nEnter size that is less than half of current size of array : ";
	cin >> x;
	while (x >= (s / 2))
	{
		cout << "Please! Enter size that is less than half of current size of array : ";
		cin >> x;
	}
	int* temp = new int[x];
	for (int i = 0;i < x;i++)
...
int main()
{
	...
	growArray(ptr, size);
	printArray(ptr, size);
	ptr=shrinkArray(ptr, size);  //that function will return pointer and store in ptr pointer
	printArray(ptr, size);
